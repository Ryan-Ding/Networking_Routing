...
with open(test_name + "/commands.txt") as f:
    ...
    for command in command_lines:
        ...
        if result[1] == "send":
            ...
            subprocess.Popen(['iptables', '-D', 'OUTPUT', '-s', source_ip, '-d', dest_ip, '-j', 'ACCEPT'])
            subprocess.Popen(['iptables', '-D', 'OUTPUT', '-s', dest_ip, '-d', source_ip, '-j', 'ACCEPT'])
        elif result[1] == "linkup":
            source_id = result[0]
            dest_id = result[2]
            source_ip = "10.1.1." + source_id
            dest_ip = "10.1.1." + dest_id
            subprocess.Popen(['iptables', '-I', 'OUTPUT', '-s', source_ip, '-d', dest_ip, '-j', 'ACCEPT'])
            subprocess.Popen(['iptables', '-I', 'OUTPUT', '-s', dest_ip, '-d', source_ip, '-j', 'ACCEPT'])
        elif result[1] == "sleep":
...
for node in nodes_list:
    ...
    if compare:
        print "log" + source_id + ".txt matched"
    else:
